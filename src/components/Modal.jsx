...
const Modal = ({ itemsOpen, closeItems, activeOrder, isAr }) => {
  ...
  const totalOfActiveOrder = useMemo(() => {
    ...
    return activeOrder.items.reduce((sum, it) => sum + Number(it.qty || 0) * Number(it.price || 0), 0);
  }, [activeOrder]);

  // coupon: node?.order?.appliedCoupon?.value,
  //           couponType: node?.order?.appliedCoupon?.value,
  //           taxAmount: node?.order?.taxAmount,
  ...
  return (
    ...
    >
      <div
        className="max-h-[70vh] overflow-y-scroll modal_box"
      ...
      >
        ...
        <DialogContent sx={{ px: 0, pb: 0 }}>
          <Box sx={{ px: { xs: 2, sm: 4 }, pt: 2, pb: 3 }}>
            {/* Sender / Receiver */}
            <Box className="flex md:flex-row flex-col gap-3" sx={{ justifyContent: "space-between", gap: 3, mt: 1, mb: 2 }}>
              <Box>
            ...
            <Box>
              {(activeOrder?.items || []).map((it, i) => (
                ...
                >
                  ...
                  <Box className="flex md:flex-row flex-col items-center gap-3">
                    <Box
                      component="img"
                      src={it.image}
                      alt={isAr ? it.ar_name : it.en_name}
                      sx={{
                        width: 36,
                        height: 36,
                        objectFit: "cover",
                    ...
                    />
                    <h6 className="md:text-base text-[.8rem] font-medium" style={{ color: "#374151" }}>
                      {isAr ? (it.ar_name || it.en_name) : (it.en_name || it.ar_name)}
                    </h6>
                  ...
                  <h5
                    style={{ color: "#111827", fontSize: ".9rem", fontWeight: 400 }}
                    className="md:text-end text-center"
                  >
                    QAR {currency((it.qty || 0) * (it.price || 0))}
                  </h5>
                </Box>
              ))}
            </Box>
            ...
            <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between" }}>
              <h6 style={{ color: PRIMARY, fontWeight: 800, fontSize: "1rem" }}>
                {isAr ? "المجموع" : "Delivery Charges"}
              </h6>
              <h5 style={{ ml: "auto", fontWeight: 400, color: "#111827", fontSize: ".8rem" }}>
                QAR {activeOrder?.taxAmount || '200'}
              </h5>
            </div>
            <div className="py-3" style={{ display: "flex", alignItems: "center", justifyContent: "space-between" }}>
              <h6 style={{ color: PRIMARY, fontWeight: 800, fontSize: "1rem" }}>
                {isAr ? "المجموع" : "Coupon Discount"}
              </h6>
              <h5 className={`${activeOrder?.couponType === "percentage" ? "text-green-600" : "text-[#111827]"} `} style={{ ml: "auto", fontWeight: 400, fontSize: ".8rem" }}>
                {activeOrder?.couponType === "fixed" && "QAR "}
                {
                  activeOrder?.coupon
                }{activeOrder?.couponType === "percentage" && "%"}
              </h5>
            </div>
            <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between" }}>
              ...
              <h5 style={{ ml: "auto", fontWeight: 400, color: "#111827", fontSize: "1rem" }}>
                QAR {activeOrder?.grandTotal}
              </h5>
            </div>
          </Box>
        </DialogContent>
