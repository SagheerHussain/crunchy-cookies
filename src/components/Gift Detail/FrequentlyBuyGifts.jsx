...
const FrequentlyBuyGifts = ({ data, product }) => {

  const { i18n } = useTranslation();
  const langClass = i18n.language === "ar";

  console.log("frequently bought together", data, product)
        ...
        >
          ...
          <div className="mt-4">
            <button className="flex items-center justify-center font-medium text-sm mt-4 bg-primary hover:bg-primary/70 text-white p-2 rounded-lg w-full">
              ...
              {langClass ? "إضافة كلاهما إلى السلة" : "Add Both to Cart"} QAR {data?.price + p?.price}
            </button>
          </div>
        </div>
      ))}
